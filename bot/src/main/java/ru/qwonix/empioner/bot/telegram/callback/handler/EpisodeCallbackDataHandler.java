...
import ru.qwonix.empioner.bot.telegram.callback.data.EpisodeCallbackData;
import ru.qwonix.empioner.bot.telegram.service.TelegramBotExecutionService;
import ru.qwonix.empioner.bot.telegram.service.TelegramEpisodeService;
import ru.qwonix.empioner.bot.telegram.service.TelegramVideoService;

import java.util.List;
import java.util.Optional;

import static ru.qwonix.empioner.bot.telegram.service.TelegramBotExecutionService.escapeMarkdownMessage;

@RequiredArgsConstructor
@Slf4j
@Component
public class EpisodeCallbackDataHandler implements CallbackDataHandler {
    ...
    public boolean canHandle(Class<? extends CallbackData> callbackData) {
        return EpisodeCallbackData.class.isAssignableFrom(callbackData);
    }

    @Override
    public void handle(TelegramBotUser user, CallbackQuery callbackQuery, CallbackData callbackData) {
        EpisodeCallbackData episodeCallbackData = (EpisodeCallbackData) callbackData;
        Optional<Episode> optionalEpisode =
                telegramEpisodeService.findById(episodeCallbackData.episodeId());

        if (optionalEpisode.isEmpty()) {
            telegramBotExecutionService.executeAlertWithText(
                    callbackQuery.getId(),
                    "Такого эпизода не существует. Попробуйте найти его заново.",
                    true
            );
            return;
        }
        ...
        Optional<Video> optionalVideo = telegramVideoService.findMaxPriorityInGroup(episode.videoGroupId());

        //todo даже если видео не найдено показать эпизод
        if (optionalVideo.isEmpty()) {
            telegramBotExecutionService.executeAlertWithText(
                    callbackQuery.getId(),
                    "Видео не найдено. Попробуйте заново немного позже",
                    false);
            return;
        }
